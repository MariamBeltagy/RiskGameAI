...
def getCostEC(g,node):
    ...
    while(i<9):
        ...
        cost = cost + math.sqrt( pow((y2-y1),2) + pow((x2-x1),2) )
        i=i+1
...
while(1):
    ...
    if (up.getState() != None and not(frontier.__contains__(up.getState()))):
        up.setG(g)
        up.setF(getCostEC(g, up.getState()))
        s.enqueue(up)
    ...
    if (down.getState() != None and not (frontier.__contains__(down.getState()))):
        down.setG(g)
        down.setF(getCostEC(g, down.getState()))
        s.enqueue(down)
    ...
    if (left.getState() != None and not (frontier.__contains__(left.getState()))):
        left.setG(g)
        left.setF(getCostEC(g, left.getState()))
        s.enqueue(left)
    ...
    if (right.getState() != None and not(frontier.__contains__(right.getState()))):
        right.setG(g)
        right.setF(getCostEC(g, right.getState()))
        s.enqueue(right)
